...
- **[P3]** `AND`/`OR` evaluate left to right with **short-circuit** — a side effect in the right operand (e.g. a call the following code relies on) is not evaluated once the left side decides the result; keep operands free of side effects.
