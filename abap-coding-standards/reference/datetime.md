...
- **[P3]** Day of week (1 = Monday): do not compute via `lv_date MOD 7`. Compute via the difference from a known Monday (`20240101`): `DATA(wd) = ( lv_date - CONV d( '20240101' ) ) MOD 7 + 1.` (`MOD` returns a non-negative remainder — also works for dates before the reference.) Or FM `DAY_IN_WEEK` (`datum` → `wotnr`, 1 = Monday).
