...
- **[P1]** Do not use `ASSERT` at all: even when active, it aborts the program with a dump and gives no useful information (where/why — not visible, unlike `RAISE` with a message). Integrity/impossible-state checks — only `IF … RAISE` with a clear text, or explicit error handling.
...
- **[P2]** HR OM/PA — via classes: `set_exclusive_lock`/`remove_exclusive_lock` (release in `CLEANUP`), write — trial pattern: `start_trial` → `approve_trial`+`flush(no_commit=abap_false)` / `discard_trial` in `CATCH`. (Interface `if_hrpa_masterdata`, class `cl_hrpa_masterdata`; verify signatures in SE24.)
...
- **[P2]** V1 (critical) / V2 (secondary: statistics, indexes, background) — registered inside the update FM (`PERFORM ... ON COMMIT` — obsolete form, same semantics). A V1 error → V2 does not run, everything rolls back; a V2 error → V1 is already committed.
- **[P2]** `COMMIT WORK AND WAIT` — synchronous: waits for update tasks, `sy-subrc` shows their result; without `AND WAIT` update tasks run asynchronously, the result is not visible. Need the write result — `AND WAIT` (in dialog — delay; if the status is not needed, `COMMIT WORK` without `AND WAIT` is faster).
...
- **[info]** `RAISE RESUMABLE EXCEPTION ...` — the caller can `CATCH ... BEFORE UNWIND` and continue via `RETRY` (repeat from the `RAISE` point). `RETRY` — only in `CATCH ... BEFORE UNWIND` of a resumable exception.
