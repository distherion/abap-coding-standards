...
- **[P2]** A file on the presentation server (frontend): `cl_gui_frontend_services=>gui_upload`/`gui_download`; large files — `gui_download` with `filetype = 'BIN'`/`'ASC'`. These methods are classic **FM with `EXCEPTIONS`** (`file_open_error`, `file_read_error`, `no_batch`, `error_no_gui`, …) — check `sy-subrc` immediately; there is **no** class-based exception for frontend file access (`cntl_error` belongs to the GUI-control classes, not to `cl_gui_frontend_services`) — do not expect a class-based `CATCH` here.
