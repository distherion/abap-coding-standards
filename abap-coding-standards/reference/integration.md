...
- **[P2]** `ctu_params-dismode`: `'N'` — background (no UI), `'E'` — only screens with an error, `'A'` — all, `'P'` — no screens, with debugging possible. In prod — `'N'`/`'E'`, not `'A'`. `updmode = 'S'` (synchronous) — when the write result matters immediately. `nobinpt = 'X'` — switch off batch-input processing (common in mass runs).
...
- **[info]** ABAP as an HTTP client — create via `cl_http_client=>create_by_url( url = ... )` (the common form); the raw `create( host = ... service = ... )` is documented as an equal alternative (ABAPDocu "cl_http_client"). Reusable connections with SSL/auth/proxy configured in SM59 — `create_by_destination( destination = ... )`. TLS relies on the SM59 destination's SSL or the default SSL client — no SSL is set in code (`ssl_id` unused).
