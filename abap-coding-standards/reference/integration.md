...
- **[P2]** Loading via a transaction — `CALL TRANSACTION <tcode> USING lt_bdcdata OPTIONS FROM ls_ctu_params MESSAGES INTO lt_msg`. `MESSAGES INTO` (table `bdcmsgcoll`) is mandatory: without it, transaction errors are silently lost. Determine success not by `sy-subrc` (can be `001` on success) but by the absence of `msqtyp = 'E'`/`'A'` in `lt_msg` (or by the transaction's success message).
...
- **[P3]** The new (kernel) BAdI syntax — `GET BADI lo_badi FILTERS filter = lv_value` + check `sy-subrc`/`CATCH cx_badi_not_implemented` + `CALL BADI lo_badi->method( )`; cleaner than the classic `GET BADI` with a proxy object. For new points — only the new syntax.
