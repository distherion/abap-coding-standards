...
- **[P2]** A missing row is normal: read directly with `VALUE #( itab[ key ] OPTIONAL )` (no row → `IS INITIAL`; `#` works here in an inline declaration too — a single table expression carries the type, `data.md`) or `VALUE #( itab[ key ] DEFAULT ls_dflt )` instead of `TRY`/`CATCH cx_sy_itab_line_not_found`; no double read — not `line_exists( )` + a repeated `READ` (if the row must exist — `TRY` + `CATCH cx_sy_itab_line_not_found` and your own exception).
