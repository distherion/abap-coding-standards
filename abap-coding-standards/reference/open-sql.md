...
- **[info]** On HANA `FOR ALL ENTRIES` is converted to **FDA** — the DB rewrites the `IN`-condition into a join instead of the classic key transfer (SAP Note 1662726 "Optimization of select with FOR ALL ENTRIES on SAP HANA database"; related parameter note 1987132 — verify in the note). FAE stays relevant on HANA; a very large RANGE `IN` can be slower than FAE — measure with ST05, do not decide by rule of thumb.
