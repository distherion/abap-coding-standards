...
- **[info]** On HANA `FOR ALL ENTRIES` is converted to **FDA** — the DB rewrites the `IN`-condition into a join instead of the classic key transfer (SAP Note 2399993, verify in the note). FAE stays relevant on HANA; a very large RANGE `IN` can be slower than FAE — measure with ST05, do not decide by rule of thumb.
