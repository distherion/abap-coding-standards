...
- **[P3]** **No obsolete statements** — where 7.50 has a modern replacement, use it (ABAPDocu "Obsolete Language Elements", Clean ABAP): `REFRESH itab` → `CLEAR itab`; `MOVE` → `=`; `MOVE-CORRESPONDING` → `CORRESPONDING #( )`; `CREATE OBJECT` → `NEW`; `TABLES`/`NODES`/`WITH HEADER LINE` → `DATA` (see `data.md`); host variables in Open SQL must be `@`-escaped (the unescaped form is obsolete).
