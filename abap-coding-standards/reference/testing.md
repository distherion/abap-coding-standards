...
- **[P2]** `TEARDOWN` — only if actually needed (cleaning DB/external resources in an integration test). Resetting `cut`/test doubles is redundant — `setup` will overwrite. Note: `setup` re-creates instance state on every test but does **not** reset class-level (static) data, buffers and global switches — reset those explicitly (`TEARDOWN` or in `setup`) or tests leak state into each other.

## Test data

- **[P3]** Make meaningless data obviously meaningless — `'42'`, `'?=/"&'`, `CONSTANTS some_random_number TYPE i VALUE 782346.`, `CONSTANTS some_nonsense_key TYPE char8 VALUE 'ABCDEFGH'.` — and make differences immediately noticeable (`END1`/`END2`), do not force finding the difference in long identical strings; do not give reason to think `'00000001223678871'` is a real object if it is not.

## Assertions

- **[P3]** A few focused asserts, exactly about the essence of the test — a pile of asserts means unclear focus and extra coupling between test and production; check the contents (rows/records), not the count (`assert_equals( act = lines( log ) exp = 3 )` is fragile — the number may vary — and blind — the number matches, the contents differ); if a property of the result matters rather than the exact contents (meta-quality), assert that instead of full equality (exact comparison is fragile under a permitted refactor). The right assert type — `assert_equals`/`assert_false`/`assert_initial`/`assert_not_initial`/`assert_bound`/`assert_subrc`/`assert_table_contains`/`assert_differs`/`fail`, not `assert_true( xsdbool( act = exp ) )` (the specific assert gives a description on mismatch).
- **[P3]** Exceptions in a test: an expected one — `TRY ... cl_abap_unit_assert=>fail( ). CATCH zcx_x. ENDTRY.` (fall via `fail` if it did not come); an unexpected one — propagate it out (`METHODS m FOR TESTING RAISING zcx_x.`), do not catch and wrap into `fail` — the test stays on the happy path, more readable.
